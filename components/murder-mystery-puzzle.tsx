...
const librarianDialogue: DialogueOption[] = [
  {
    id: "initial-greeting",
    text: "Start",
    response: "...",
...
const MurderMysteryPuzzle = forwardRef(
  ({ onSolve, onLocationChange, currentQuestion }: MurderMysteryPuzzleProps, ref) => {
    ...
    // Dialogue flags - for conditional dialogue options
    const [dialogueFlags, setDialogueFlags] = useState<{ [key: string]: boolean }>({
      "asked-about-murder": false,
      "seen-police-report": false,
      "seen-passport": false,
      "body-not-accessible": true,
      "can-see-body": false,
    ...
    const updateDialogueFlag = (flag: string, value: boolean) => {
      setDialogueFlags((prev) => ({
        ...prev,
        [flag]: value,
      }))
    }

    const resetDialogueFlags = () => {
      setDialogueFlags({
        "asked-about-murder": false,
        "seen-police-report": false,
        "seen-passport": false,
      ...
      // Reset all the other related state as well
      setShowPoliceReport(false)
      setShowPassport(false)
      setShowVictimBody(false)
      setShowAutopsyReport(false)
      setAskedAboutFriends(false)
      setCanSeeBody(false)
      setHasCheckedBody(false)
      setAskedHobbies(false)
      setAskedPuzzleGames(false)
      setKnowsAboutAnemia(false)
      setKnowsAboutBodyMarks(false)
      setDemonologyBookOpened(false)

      // Reset the dialogue system too
      if (dialogue && dialogue.resetDialogue) {
    ...
    const filterPoliceOptions = (options: any[]) => {
      return options.filter((opt) => {
        ...
        if (opt.id === "were-there-any-witnesses") {
          return dialogueFlags["asked-about-murder"]
        }
        if (opt.id === "can-see-report-again") {
          return dialogueFlags["seen-police-report"]
        }
        if (opt.id === "can-see-passport-again") {
          return dialogueFlags["seen-passport"]
        }
        return true
      })
    }
    ...
    const handleDialogueOption = (option: any) => {
      ...
      if (option.id === "any-friends") {
        ...
        dialogue.setLastAction("viewed-report")
        updateDialogueFlag("seen-police-report", true)
      } else if (option.id === "can-see-passport" || option.id === "can-see-passport-again") {
        setShowPassport(true)
        dialogue.setLastAction("viewed-passport")
        updateDialogueFlag("seen-passport", true)
    ...
    // Reset all dialogue flags when component mounts or level changes
    useEffect(() => {
      resetDialogueFlags()
            ...
            {(currentLocation === "police station" || currentLocation === "morgue" || currentLocation === "library") &&
              dialogue.showDialogue && (
                <DialogueInterface
                  ...
                  onUpdateFlags={updateDialogueFlag}
                  onGoBack={() => {
                    if (dialogue.currentCharacter === "policewoman") {
