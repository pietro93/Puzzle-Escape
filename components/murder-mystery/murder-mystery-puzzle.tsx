...
export default function MurderMysteryPuzzle({ onSolve, onLocationChange, currentQuestion }: MurderMysteryPuzzleProps) {
  ...
  // Dialogue option visibility state
  const [showPoliceReportOptionState, setShowPoliceReportOptionState] = useState(true)
  const [showWitnessesOptionState, setShowWitnessesOptionState] = useState(true)
  const [showPoliceReportAgainOptionState, setShowPoliceReportAgainOptionState] = useState(true)
  const [showPassportAgainOptionState, setShowPassportAgainOptionState] = useState(true)
  ...
  const filterPoliceOptions = (options: any[]) => {
    return options.filter((opt) => {
      if (opt.id === "police-report") {
        return showPoliceReportOptionState
      }
      if (opt.id === "were-there-any-witnesses") {
        return showWitnessesOptionState
      }
      if (opt.id === "can-see-report-again") {
        return showPoliceReportAgainOptionState
      }
      if (opt.id === "can-see-passport-again") {
        return showPassportAgainOptionState
      }
      return true
    })
  }
